/**
 * Global API and Fetch error handler for WulinMaster.
 */

/**
 * Handles errors from Fetch API requests.
 */
export function handleApiError(xhr: any) {
  let msg = xhr.responseText;
  let wulin_oauth;

  const contentType = typeof xhr.getResponseHeader === 'function' 
    ? xhr.getResponseHeader("content-type") 
    : xhr.headers?.get("content-type");

  const isJson = /application\/json/i.test(contentType);
  
  if (isJson && xhr.responseJSON) {
    msg = xhr.responseJSON.msg;
    wulin_oauth = xhr.responseJSON.wulin_oauth;
  }

  const win = window as any;

  switch (xhr.status) {
    case 401:
      const error_message = wulin_oauth ? msg : `You are not authorized to do this operation, please contact admin to update your permission.\n${msg || ''}`;
      win.displayErrorMessage(error_message, "Permission denied");
      break;
    case 500:
      win.displayErrorMessage("An unexpected server error occurred.");
      break;
    default:
      if (xhr.status >= 400) {
        win.displayErrorMessage(msg || "An error occurred during the request.", `Error ${xhr.status}`);
      }
      break;
  }
}

// Export to window for global access
(window as any).handleAjaxError = handleApiError;
